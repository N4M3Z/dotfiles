...
description: "Agent sessions post to GitHub as the N4M3Z GitHub App (rendered runewright[bot]): harness-run exports a GH_TOKEN minted by gh-app-token from the app's private key in pass, so PRs and comments are visibly agent-authored while review and merge stay with the human account. App = agent identity, private key = machine credential, individually revocable."
type: adr
category: security
tags:
    - github
...
3. **GitHub App installation tokens** — an owned app (`N4M3Z`, rendered `runewright[bot]`); JWT from a per-machine private key exchanges for 1-hour installation tokens scoped to selected repos.
...
- [+] Agent PRs, bodies, and comments render as `runewright[bot]`; the human account's actions remain unambiguously human.
